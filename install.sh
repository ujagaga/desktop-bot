...
[[ -f "$APP_DIR/server.py" && -f "$APP_DIR/requirements-server.txt" ]] || fail "Project files are missing."
...
as_user "$VENV_DIR/bin/python" -m pip install --only-binary=:all: -r "$APP_DIR/requirements-server.txt"
