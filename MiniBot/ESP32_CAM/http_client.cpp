...
static bool saveTargetVersion(uint32_t version) {
  Preferences prefs;
  if (!prefs.begin("camOTA", false)) return false;
...
static void loadTargetVersion() {
  Preferences prefs;
  if (!prefs.begin("camOTA", true)) return;
  uint32_t target = prefs.getUInt("target", 0);
  prefs.end();
  if (target > FIRMWARE_VERSION) {
    invalidVersion = target;
    LOG_printf("OTA: target %lu was not installed; marking invalid\n", (unsigned long)target);
  } else if (target && prefs.begin("camOTA", false)) {
...
static bool getText(const String &url, const char *accept, size_t limit, String &body) {
  ...
  if (!ok) LOG_printf("OTA: HTTP request failed (status %d, size %d)\n", status, size);
  http.end();
  return ok;
}
...
static bool checkRepository() {
  ...
  if (!parseFirmwareVersion(config.c_str(), remoteVersion)) {
    LOG_append("OTA: invalid remote FIRMWARE_VERSION; skipping");
    return false;
  }
  ...
  updater.onStart([]() { LOG_append("OTA: downloading image"); });
  updater.onProgress([](int current, int total) {
    static int lastStep = -1;
    if (current == 0) lastStep = -1;
    int step = total > 0 ? (int)((uint64_t)current * 10 / total) : 0;
    if (step != lastStep) { lastStep = step; LOG_printf("OTA: %d%%", step * 10); }
  });
  ...
  if (updater.update(client, base + "/build/ESP32_CAM.ino.bin") != HTTP_UPDATE_OK) {
    LOG_printf("OTA: update failed: %s\n", updater.getLastErrorString().c_str());
    CAM_Init();
    HTTPSRV_init();
    return false;
  }
  if (!saveTargetVersion(remoteVersion)) {
    ...
    LOG_printf("OTA: target save failed; boot restore status %d\n", (int)restored);
    CAM_Init();
    HTTPSRV_init();
    return false;
  }
  LOG_append("OTA: installed; restarting");
...
void HTTPC_process() {
  bool connected = connectedEvent.exchange(false);
  ...
  if (connected || manual) {
    ...
    LOG_append("OTA: check scheduled; waiting for network time");
  }
  if (!pending || WiFi.status() != WL_CONNECTED ||
      (int32_t)(millis() - nextAttempt) < 0) return;

  // Do not bypass TLS validation before NTP has supplied a valid clock.
